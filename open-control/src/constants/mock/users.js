export default [
  /*
   * entrepreneurs
   */
  {
    id: 1,
    name: 'Иванов Иван Иванович',
    email: 'i.ivanov@outlook.com',
    phone: '+79990009900',
    avatar:
      'https://media.licdn.com/dms/image/C4E03AQGw1v_FxUk8Nw/profile-displayphoto-shrink_800_800/0/1658746900948?e=2147483647&v=beta&t=KhqtB9qrPPz72p9ENd_CZCVRg929x7q1tcebeUlREak',
    position: 'Генеральный директор',
    roles: ['entrepreneur'],
  },
  {
    id: 2,
    name: 'Петров Петр Петрович',
    email: 'p.petrov@icloud.com',
    phone: '+79123456789',
    avatar:
      'https://media.licdn.com/dms/image/C5603AQHaaK2HrfETOQ/profile-displayphoto-shrink_800_800/0/1636901431703?e=2147483647&v=beta&t=dNFuP5ZG20qjCaUkdXuOGxeQHKcCviL9HaiUaHB1t5Y',
    position: 'Генеральный директор',
    roles: ['entrepreneur'],
  },
  {
    id: 3,
    name: 'Федорова Анастасия Алексеевна',
    email: 'a.fedorova@mail.ru',
    phone: '+79259876543',
    avatar:
      'https://media.istockphoto.com/id/1317804578/photo/one-businesswoman-headshot-smiling-at-the-camera.jpg?s=612x612&w=0&k=20&c=EqR2Lffp4tkIYzpqYh8aYIPRr-gmZliRHRxcQC5yylY=',
    position: 'Генеральный директор',
    roles: ['entrepreneur'],
  },
  {
    id: 4,
    name: 'Новиков Артем Сергеевич',
    email: 'a.novikov@gmail.com',
    phone: '+79043216543',
    avatar: 'https://jooinn.com/images/businessman-portrait-1.jpg',
    position: 'Генеральный директор',
    roles: ['entrepreneur'],
  },

  /*
   * employees
   */
  {
    id: 5,
    name: 'Иванова Дарья Сергеевна',
    email: 'd.ivanova@yandex.ru',
    phone: '+79156789012',
    avatar:
      'https://img.freepik.com/premium-photo/real-professional-smiling-businesswoman-looking-confident-determined-face-expression-standing-sui_1258-88420.jpg',
    position: 'Юрист',
    roles: ['employee'],
  },
  {
    id: 6,
    name: 'Григорьев Василий Алексеевич',
    email: 'v.grigoryev@hotmail.com',
    phone: '+79112345678',
    avatar:
      'https://media.istockphoto.com/id/464621546/photo/successful-mature-man-smiling-looking-away.jpg?s=612x612&w=0&k=20&c=QNkzhwIQsB2L2PenwIRRhgBLN9-bQFGMudGFKdLww6w=',
    position: 'Юрист',
    roles: ['employee'],
  },
  {
    id: 7,
    name: 'Смирнова Елена Владимировн',
    email: 'e.smirnova@yahhoo.com',
    phone: '+79123456789',
    avatar:
      'https://media.istockphoto.com/id/1329936184/photo/head-shot-portrait-of-smiling-businesswoman-intern-looking-at-camera.jpg?s=612x612&w=0&k=20&c=cCeWt0GYUtcFsGFi6hTFci-d5KZY6FAd_UchY9V5aSo=',
    position: 'Главный бухгалтер',
    roles: ['employee'],
  },
  {
    id: 8,
    name: 'Кузнецова Ольга Александровна',
    email: 'o.kuznetsova@gmail.com',
    phone: '+79031234567',
    avatar:
      'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR-68u3o7Re4BPg-Lo8oolbxZ9AubEv5Aai9A&usqp=CAU',
    position: 'Главный бухгалтер',
    roles: ['employee'],
  },
  {
    id: 9,
    name: 'Павлова Мария Александровна',
    email: 'm.pavlova@hotmail.com',
    phone: '+79045678901',
    avatar: '',
    position: 'Главный бухгалтер',
    roles: ['employee'],
  },
  {
    id: 10,
    name: 'Дмитриев Дмитрий Дмитриевич',
    email: 'd.dmitriev@gmail.com',
    phone: '+79193216543',
    avatar: '',
    position: 'Финансовый директор',
    roles: ['employee'],
  },
  {
    id: 11,
    name: 'Романова Екатерина Владимировна',
    email: 'e.romanova@mail.ru',
    phone: '+79119012345',
    avatar:
      'https://www.businessofeminin.com/wp-content/uploads/2019/10/Photo-8-conseils-beaute-pour-Business-Women.jpg',
    position: 'Финансовый директор',
    roles: ['employee'],
  },

  {
    id: 12,
    name: 'Михайлов Михаил Михайлович',
    email: 'm.mihailov@yandex.ru',
    phone: '+79045678901',
    avatar:
      'https://media.istockphoto.com/id/1303206558/photo/headshot-portrait-of-smiling-businessman-talk-on-video-call.jpg?s=612x612&w=0&k=20&c=hMJhVHKeTIznZgOKhtlPQEdZqb0lJ5Nekz1A9f8sPV8=',
    position: 'Специалист по закупкам',
    roles: ['employee'],
  },

  /*
   * inspectors
   */
  {
    id: 13,
    name: 'Иванов Алексей Сергеевич',
    email: 'a.ivanov@knd.mos.ru',
    phone: '+79118528503',
    avatar:
      'https://t4.ftcdn.net/jpg/01/81/21/31/360_F_181213119_ZL1PrnT9k0dvZn8uhBRqnWucK2xM5mKU.jpg',
    position: 'Инспектор',
    roles: ['inspector'],
  },
];
